...
The prototype hero now puts the main player verbs before the debug-heavy control grid: Continue, Look, Move, Ask, Talk, Objects, Handling, Proposals, Ask proposal, Support, Wait proposal, Wait, Save, Return, Physics path, Follow, and Space.
...
Player Mode now exposes this as a visible `visible physics follow continuity` card. The player-facing surface and receipt summarize the latest Follow row, Physics path action, happy-path id, save/return counts, body-expression id, and completion attempts. Player Mode readiness now depends on that card, so the visible Follow continuity cannot remain only an acceptance/export artifact.

QA readiness and exported acceptance now also require that player-facing card. `QA readiness` reports `player-mode-visible-physics-follow`, and exported acceptance includes `player_mode_visible_physics_follow_continuity` beside `player_mode_interface`. The first playable gate therefore checks the same continuity in the normal surface, Player Mode receipt, and acceptance export.

The visible Follow card now participates in save/return continuity. Prototype save slots store the card count, Follow id, Physics path action id, happy-path id, save/return counts, body-expression id, and fingerprint. Return logs recompute the restored Player Mode visible-Follow card and compare that fingerprint. QA readiness reports `player-mode-visible-physics-follow-save-return`, and exported acceptance includes `player_mode_visible_physics_follow_save_return_continuity`.

The Return journal now surfaces the same restored Player Mode visible-Follow continuity. Journal snapshots and rows show the saved/restored Follow id, Physics action id, body-expression id, and match state in the normal player return surface. Prototype QA, QA readiness, and exported acceptance require this through `return-journal-visible-physics-follow` and `return_journal_visible_physics_follow_continuity`.
