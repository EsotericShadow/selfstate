...
Current SSRM-3D coupled crisis temporal-return-value evidence does not satisfy item 73: temporal labels distinguish stronger joint rollouts from return-selected rollouts, and validation selects value bias `2.5`, but held-out crisis score remains `0.000`, total score is `0.518` versus `0.520` for the return-selected GRU and `0.652` for fixed joint, and fixed joint keeps far stronger resolved rate and coupled response. This says delayed labels alone still do not replace active policy learning.
