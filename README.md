...
- [Dynamic WrongFix Arena report](docs/140_dynamic_wrongfix_arena_report.md): extends that bridge with 120 seeded tasks plus 5 deterministic false-positive calibration tasks (125 total), held-out families, noisy and irrelevant signals, difficulty tiers, and additional policies; `weighted_quality_critic` bests the visible baseline on hidden pass, wrong-fix, root-cause, and regression avoidance with zero false-positive and zero overblocking in this run.
