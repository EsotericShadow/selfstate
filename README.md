...
- [Primary demo review evidence refresh report](docs/324_ssrm_3d_browser_world_v84_primary_demo_review_evidence_refresh_report.md): lets the outside-review launcher refresh and export shell-side replay, receipt, observation, checkpoint, and replay-export evidence after a walkthrough.
...
- [Primary demo cross-tab handoff continuity report](docs/335_ssrm_3d_browser_world_v95_primary_demo_cross_tab_handoff_continuity_bridge_report.md): verifies that a fresh launcher tab can see the same prepared resume handoff, continue into the shell, return, refresh evidence, and preserve the visible timestamp without raw storage inspection.
- [Primary demo closed-origin-tab handoff continuity report](docs/336_ssrm_3d_browser_world_v96_primary_demo_closed_origin_tab_handoff_continuity_bridge_report.md): verifies that the prepared resume handoff remains visible and usable from a fresh launcher tab after the original preparing tab is closed.
...
Report 301 hardens the Report 300 vertical slice into a maintained browser app shell instead of adding another simulation organ. It separates `index.html`, `styles.css`, `app.js`, `playtest_tasks.json`, `qa_manifest.json`, and an operator README under `visualizations/ssrm_3d_browser_world_v61_vertical_slice_app_shell/`. The deterministic run passes with readiness `0.947050`, mean channel score `0.983500`, and weakest-channel score `0.862000`. It records 6 app shell files, 10 playtest tasks, 9 state boundary rules, 6 direct QA hooks, and only 6 generated artifact files.

The weakest channel is `not_runtime_browser_verified_yet` at `0.862000`. That is intentional pressure: the app shell now contains browser-callable QA hooks, but the direct browser QA pass is still the next gate. Artifacts: `experiments/ssrm_3d_browser_world_v61_vertical_slice_app_shell_hardening.py`, `artifacts/ssrm_3d_browser_world_v61_vertical_slice_app_shell_hardening_results.json`, `visualizations/ssrm_3d_browser_world_v61_vertical_slice_app_shell/index.html`, and `docs/301_ssrm_3d_browser_world_v61_vertical_slice_app_shell_hardening_report.md`. Boundary: Deterministic browser-local hardened vertical-slice app shell only; no LLM call, subjective consciousness, real consent, autonomous natural language, moral patienthood, production persistence, finished gameplay, complete 3D engine, or metaphysical frequency claim. Next gate: post-301 direct browser QA pass: open the maintained app shell, execute the built-in playtest checklist, inspect saved localStorage state, export replay, and fix runtime issues before adding any new simulation organs.


### Report 302: SSRM-3D Browser World v62 App-Shell Direct Browser QA

Report 302 runs the first direct browser QA pass against the maintained v61 app shell. The pass opened the shell through localhost, drove 17 UI actions, verified 10 / 10 playtest checks, confirmed two passing state-boundary audits, prepared replay export without relying on unsupported browser downloads, and found zero console errors. The deterministic evidence summary passes with readiness `0.952000` and weakest-channel score `0.868000`. Runtime fixes included the missing `Ask schedule` button, QA-friendly replay export, sanitized state-boundary audit, and `?reset=1` clean-start path.

The weakest channel is `single_browser_run_not_playtest_cohort` because this is one automated browser pass, not an external playtest cohort. Artifacts: `experiments/ssrm_3d_browser_world_v62_app_shell_direct_browser_qa.py`, `artifacts/ssrm_3d_browser_world_v62_app_shell_direct_browser_qa_results.json`, `artifacts/ssrm_3d_browser_world_v62_app_shell_direct_browser_qa_browser_evidence.json`, and `docs/302_ssrm_3d_browser_world_v62_app_shell_direct_browser_qa_report.md`. Boundary: Direct browser QA evidence over the deterministic browser-local v61 app shell only; no LLM call, subjective consciousness, real consent, autonomous natural language, moral patienthood, production persistence, finished gameplay, complete 3D engine, or metaphysical frequency claim. Next gate: post-302 hardening: package the maintained app shell as the primary demo entry point, add a minimal manual playtest script, and reduce future work to defects found in the single playable shell before adding any new generated report organs.

## Report 303: Browser World v63 Primary Demo Entrypoint Manual Playtest Package

Report 303 packages the maintained v61 app shell as the stable primary browser-world demo entrypoint instead of creating another parallel world artifact. It adds `visualizations/ssrm_3d_browser_world_primary_demo/index.html`, a manual playtest script, launch manifest, and QA manifest that point back to the same shell directly tested in Report 302. The deterministic packaging run passes with readiness `0.950420` and weakest-channel score `0.866000`. The honest limit is `manual_playtest_not_external_cohort`: the demo is easier to launch and review, but this is not an outside playtest cohort, production deployment, finished game, autonomous language system, or consciousness claim.
...
This is not open-ended language and does not call an LLM. It makes residents more readable during normal play while preserving source history, no direct command, no hidden-law normal view, and no tech-tree unlock boundaries.
