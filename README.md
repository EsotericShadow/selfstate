...
Current software repair bridge evidence supports item 92 only as a structured arena bridge: `15` toy repair tasks show that `visible_test_only` passes visible tests but gets hidden pass rate `0.067`, wrong-fix rate `1.000`, and weakest-channel score `0.012`. `min_channel_critic` reaches hidden pass rate `1.000`, wrong-fix rate `0.000`, root-cause repair rate `1.000`, and weakest-channel score `0.864`. This is not real repo coding or proof of frontier LLM improvement.
