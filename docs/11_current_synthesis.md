...
The next practical SSRM-3D step is narrower than the full attractor test: replace single-step crisis value labels with temporally extended consequence learning. The trained recurrent policy should act in the simulated world, receive delayed consequences, learn repair timing and wrong-channel cost across the full crisis window, and then fail under targeted social and environmental ablations while acting. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
