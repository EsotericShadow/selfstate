...
The next practical SSRM-3D step is narrower than the full attractor test: add development and skill learning after the structured perception, sleep-rest, illness/sanitation, weather/exposure, tool/shelter degradation, social trust/contracts, predator/threat, resource-ecology, and injury/disability layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
