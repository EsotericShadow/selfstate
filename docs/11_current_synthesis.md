...
The next practical SSRM-3D step is narrower than the full attractor test: add places with function and territory/social infrastructure after the structured perception, sleep-rest, illness/sanitation, weather/exposure, tool/shelter degradation, social trust/contracts, predator/threat, resource-ecology, injury/disability, development/skill, dependent-care, irreversible-loss, and affective-control layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
