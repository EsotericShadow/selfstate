...
The next practical SSRM-3D step is narrower than the full attractor test: change the training pressure so one-channel crisis response cannot survive selection. The learned controller already has active actions, a critic baseline, and recurrent crisis memory; what it lacks is consequence training that preserves both social and environmental repair channels. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
