...
The next practical SSRM-3D step is narrower than the full attractor test: add weather/exposure after the structured perception, sleep-rest, and illness/sanitation layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
