...
The newest evidence is Report 123, a bounded minimum-channel planning pass after the failed two-channel process-pressure controller. Report 106 showed that the designed controller resolves post-12h coupled crises while the learned return-selected GRU preserves generic maturation but fails crisis repair. Report 107 adds a supervised repair critic around that GRU, and validation selects repair bias `0.0`. Report 108 adds a counterfactual action-value critic; validation selects nonzero value bias `1.75`, but held-out crisis score stays `0.000` and the controller underperforms the return-selected GRU. Report 109 moves the target to sequence-window plans; validation selects plan bias `4.0`, held-out crisis score rises to `0.304`, resolved rate rises to `0.500`, and coupled response rises to `0.434`, but environmental ablation remains too weak. Report 110 makes generic environmental activity insufficient and neutralizes missing-sensor ablation artifacts; the designed controller still solves the stricter task, but the learned overlay keeps crisis score at `0.000`. Report 111 replaces the analytic plan target with short cloned simulator rollouts, and validation rejects the rollout overlay by selecting plan bias `0.0`. Report 112 trains a recurrent diagnostic head to predict the primary environmental repair action directly; offline diagnostic accuracy reaches `0.991`, but validation still selects diagnostic bias `0.0` because nonzero bias raises environmental response while killing social response and increasing damage. Report 113 then trains separate recurrent environmental and social action heads and validation-selects joint arbitration. Held-out crisis score rises from `0.000` to `0.380`, resolved rate rises from `0.100` to `0.650`, and coupled response rises from `0.027` to `0.646`; social/culture and environment ablations both collapse crisis score and coupled response. Report 114 extends that bridge to 96h worlds with randomized post-12h crisis timing, order, repetition, and initial world pressure. Eval worlds average `5.8` post-gate crises; crisis score rises from `0.000` to `0.706` over the return-selected GRU, resolved rate rises from `0.067` to `0.967`, coupled response rises from `0.055` to `0.965`, and both social/culture and environment ablations collapse crisis and coupled response. Report 115 replaces fixed quota-grid selection with compact return-searched adaptive allocation. It improves over the return-selected GRU in that run, but the stronger non-fixed-transfer gate fails because crisis score remains too low. Report 116 then trains a value selector from closed-loop allocator-policy consequences. The value selector improves tune objective from `1.186` to `1.354`, but the selected policy transfers worse than the seed/fixed allocator on held-out worlds: crisis score is `0.224` versus `0.361`, resolved rate is `0.627` versus `0.727`, and coupled response is `0.618` versus `0.701`. Report 117 moves value learning into active crisis observations plus candidate repair actions. The value head trains on `120000` examples and validation selects bias `2.5`, but held-out crisis score remains `0.000`, total score is `0.518` versus `0.520` for return-selected and `0.604` for fixed joint, and resolved rate is `0.140` versus `0.620` for fixed joint. Report 118 labels active-crisis actions by later crisis-window outcomes. Those labels distinguish stronger joint policies from return-selected rollouts, but held-out crisis score still remains `0.000`; resolved rate improves to `0.280` over return-selected `0.033`, but fixed joint is still far stronger at `0.687` resolved rate and `0.279` crisis score. Report 119 then trains a sampled policy inside active crisis windows. It trains over `73` completed crises and raises coupled response from `0.028` to `0.147` over the return-selected GRU, but held-out crisis score remains `0.000`, total score only improves by `0.0003`, and fixed-joint coordination remains far stronger with `0.349` crisis score and `0.693` coupled response. Report 120 adds a critic/value baseline to that active policy. The critic trains over `71` crises, but held-out actor-critic crisis score, resolved rate, and coupled response all collapse to `0.000`; total score falls to `0.504` versus `0.520` for active policy and `0.594` for fixed-joint coordination. Report 121 carries recurrent hidden state through each active crisis sequence and trains over `90` completed crises. Held-out crisis score, resolved rate, and coupled response still collapse to `0.000`; the controller loses the active policy's partial coupled response and remains far below fixed-joint coordination. Report 122 adds explicit two-channel process pressure and trains over `91` crises. It still fails: held-out crisis score, resolved rate, and coupled response remain `0.000`; the selected process policy collapses to social response `1.000` and environmental response `0.000`, while fixed-joint coordination remains far stronger with crisis score `0.432` and coupled response `0.731`. Report 123 then replaces fixed quotas with a dynamic weakest-channel planner around learned environmental/social heads. It passes the bounded transfer gate: held-out randomized 96h worlds average `5.667` post-gate crises, crisis score is `0.590`, resolved rate is `0.878`, coupled response is `0.828`, and social/culture plus environment ablations both collapse crisis score and coupled response.

Reports 93-123 form the bridge into that result. Long-horizon adaptation, hidden regime shifts, learned hidden-regime control, option gating, return-selected bias tuning, focused social/culture regimes, mutually exclusive social repair opportunity costs, repair-critic control, multi-day maturation, learned multi-day action selection, return-selected multi-day pressure routing, coupled social/environment crisis repair, coupled repair-critic reranking, coupled outcome-value reranking, coupled sequence-outcome control, coupled environmental-bottleneck control, coupled rollout-window control, coupled diagnostic-memory control, coupled joint-arbitration control, coupled randomized-transfer control, coupled adaptive-allocation control, coupled policy/value-allocation control, active state/action value control, temporal-return value control, sampled active-policy control, actor-critic value-baseline control, recurrent crisis-memory policy control, two-channel process-pressure control, and minimum-channel dynamic planning all add pressure. Report 97 remains the strongest general hidden-regime learned-control pass. Report 98 exposes the remaining social/culture weakness, Report 99 turns that weakness into a sharper negative result, Report 101 shows that a repair critic helps but does not solve causal social repair, Report 103 expands the long-run world mechanics, Report 104 shows that a GRU can preserve the 72h development pattern under closed-loop imitation, Report 105 shows that validation-return pressure routing still does not force clean social/environment dependence, Report 106 shows that coupled post-gate crises break the current learned controller, Report 107 shows that a supervised repair critic is rejected by validation, Report 108 shows that per-step outcome-value reranking can overfit validation while worsening held-out coupled crisis repair, Report 109 shows that sequence-window control materially improves held-out crisis repair while still failing the clean environment-dependency gate, Report 110 shows that stricter non-substitutable environmental repair still defeats the analytic sequence overlay, Report 111 shows that cloned rollout labels alone still are not enough when validation can reject the overlay, Report 112 shows that high offline diagnostic-label accuracy still does not count if using the label worsens coupled online consequences, Report 113 shows that preserving both repair channels through learned joint arbitration can finally pass the stricter coupled-crisis dependency gate, Report 114 shows that this structured bridge is not only a fixed-schedule artifact, Report 115 shows that compact return-searched allocation helps but is not yet strong enough to replace structured coordination, Report 116 shows that a value selector over allocator policies can overfit tune-world consequences and fail held-out seed-transfer improvement, Report 117 shows that single-step active crisis value labels still do not solve delayed coupled repair, Report 118 shows that passive delayed labels still do not replace active policy learning, Report 119 shows that lightweight sampled policy gradients improve coupled response but still do not recover nonzero crisis score, Report 120 shows that simply adding a critic baseline can train tightly while worsening held-out repair behavior, Report 121 shows that recurrent crisis memory alone still cannot preserve both repair channels, Report 122 shows that naive two-channel process supervision can flip the collapse from environmental-only to social-only without producing coupled repair, and Report 123 shows that a structured weakest-channel planner can recover coupled repair while staying close to fixed-joint coordination.
...
The next practical SSRM-3D step is narrower than the full attractor test: distill the successful minimum-channel planner into an active learned policy and then remove the planner at evaluation. The learned controller already has active actions, a critic baseline, recurrent crisis memory, explicit process pressure, and now a structured dynamic planner that works. What remains missing is an end-to-end learned policy that preserves the same coupled trajectories without engineered weakest-channel allocation. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
