...
The newest evidence is Report 124, a negative planner-distillation boundary after the bounded Report 123 minimum-channel planning pass. Reports 106-122 showed that return-selected learned control, repair critics, outcome-value reranking, rollout labels, diagnostic memory, sampled active policies, actor-critic baselines, recurrent crisis memory, and explicit two-channel process pressure still failed to preserve coupled environmental/social crisis repair. Report 123 then showed that a structured dynamic weakest-channel planner around learned environmental/social heads can solve the bounded transfer gate: held-out randomized 96h worlds average `5.667` post-gate crises, crisis score is `0.590`, resolved rate is `0.878`, coupled response is `0.828`, and social/culture plus environment ablations both collapse crisis score and coupled response. Report 124 distills those planner trajectories into a recurrent active-crisis policy, but planner removal fails: imitation accuracy is `0.986`, held-out crisis score remains `0.000`, resolved rate is only `0.289`, and coupled response is only `0.179`.

Reports 93-124 form the bridge into that result. Long-horizon adaptation, hidden regime shifts, learned hidden-regime control, option gating, return-selected bias tuning, focused social/culture regimes, social repair opportunity costs, multi-day maturation, return-selected multi-day pressure routing, coupled social/environment crisis repair, joint arbitration, randomized transfer, adaptive allocation, active crisis policy learning, recurrent crisis memory, two-channel process pressure, minimum-channel dynamic planning, and planner-to-policy distillation all add pressure. The current lesson is sharp: structured weakest-channel planning can recover coupled repair, but offline imitation of that planner is still not enough to retire engineered allocation. Report 97 remains the strongest general hidden-regime learned-control pass, Report 123 remains the strongest coupled-crisis structured-planning pass, and Report 124 is the newest learned-control failure boundary.
...
The next practical SSRM-3D step is narrower than the full attractor test: train the crisis policy on closed-loop consequences of its own actions, not just planner imitation. The learned controller already has active actions, a critic baseline, recurrent crisis memory, explicit process pressure, a structured dynamic planner that works, and now a failed planner-distillation boundary. What remains missing is an end-to-end learned policy that preserves coupled environmental/social repair without engineered weakest-channel allocation. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
