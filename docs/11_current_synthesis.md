...
- SSRM-3D persistent pressure layers from structured perception through irreversible loss;
...
The next practical SSRM-3D step is narrower than the full attractor test: add affective control state after the structured perception, sleep-rest, illness/sanitation, weather/exposure, tool/shelter degradation, social trust/contracts, predator/threat, resource-ecology, injury/disability, development/skill, dependent-care, and irreversible-loss layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
