...
The SSRM-3D tool-making, social, contract, threat, and resource-ecology precursors add that self-state pressure is not only internal. External markers, shared tools, identity memory, reputation, costly signals, names, obligations, alarms, shelter, trace control, caches, restraint, and territory access matter only when they preserve future options under embodied confusion, repeated social memory, delayed obligations, hostile tracking, or delayed resource consequences.
