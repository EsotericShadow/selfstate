...
The next practical SSRM-3D step is narrower than the full attractor test: train the crisis policy on closed-loop consequences of its own actions, not just planner labels. The learned controller already has active actions, a critic baseline, recurrent crisis memory, explicit process pressure, a structured dynamic planner that works, failed planner-distillation evidence, and now failed closed-loop recovery relabeling evidence. What remains missing is an end-to-end learned policy or value/process critic that preserves coupled environmental/social repair without engineered weakest-channel allocation. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
