...
The next practical SSRM-3D step is narrower than the full attractor test: add predator/threat agents after the structured perception, sleep-rest, illness/sanitation, weather/exposure, tool/shelter degradation, and social trust/contracts layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
