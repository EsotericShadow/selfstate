...
The downstream LLM-facing step is later: distill robust simulated agents into bounded critics for LLM search only after those agents transfer across seeds, maps, hidden pressures, counterfactual traces, and ablations. The current Report 112 failure shows why that later critic needs stronger consequence-trained action selection before it can be trusted: an offline-correct diagnostic head is not enough if validation turns it off or if it disrupts other required repair channels.
