...
The next practical SSRM-3D step is narrower than the full attractor test: add injury/disability adaptation after the structured perception, sleep-rest, illness/sanitation, weather/exposure, tool/shelter degradation, social trust/contracts, predator/threat, and resource-ecology layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
