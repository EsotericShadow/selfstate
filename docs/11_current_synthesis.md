...
The next practical SSRM-3D step is narrower than the full attractor test: add hunger/thirst plus illness/sanitation after the structured perception and sleep-rest layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
