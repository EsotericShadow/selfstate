...
The next practical SSRM-3D step is narrower than the full attractor test: move from structured arbitration and quota-selected overlays to direct consequence-trained repair policy. The trained recurrent policy should act in the simulated world, receive consequences, train by actor-critic, model-based return learning, or learned active-crisis memory over repair windows, and then fail under targeted social and environmental ablations while acting. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
