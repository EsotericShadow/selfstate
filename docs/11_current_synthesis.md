...
The newest software-facing bridge starts in Report 139 and is now extended in Report 140. Report 139 adds a deterministic `15`-task WrongFix Arena where several repairs can pass visible tests while failing hidden correctness channels. `visible_test_only` reaches visible pass rate `1.000` but hidden pass rate `0.067`, wrong-fix rate `1.000`, and weakest-channel score `0.012`. Report 140 extends it to `120` seeded tasks plus 5 false-positive calibration tasks (125 total) with held-out families, noisy/irrelevant signals, and explicit risk-policy tradeoffs. In Report 140, `visible_test_only` has hidden pass `0.344`, wrong-fix `0.864`, and weakest-channel `0.060022`, while `weighted_quality_critic` reaches hidden pass `1.000`, wrong-fix `0.008`, and root-cause `0.968` with zero false-positive and overblocking.
