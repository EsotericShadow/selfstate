...
The next practical SSRM-3D step is narrower than the full attractor test: add social trust/contracts after the structured perception, sleep-rest, illness/sanitation, weather/exposure, and tool/shelter degradation layers. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
