...
The next practical SSRM-3D step is narrower than the full attractor test: close the learned-control loop inside the physics kernel. The trained recurrent policy should act in the simulated world, receive consequences, train by return or model-based RL, and then fail under targeted ablations while acting. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
