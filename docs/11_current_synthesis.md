...
The next practical SSRM-3D step is narrower than the full attractor test: add day/night plus sleep/rest after the structured perception layer. The rule remains that a variable stays only if removing it changes policy, memory, tools, social behavior, or continuity.
