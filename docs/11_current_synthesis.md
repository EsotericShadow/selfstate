...
The next practical SSRM-3D step is narrower than the full attractor test: replace analytic repair-window bias with consequence-trained repair-window control. The trained recurrent policy should act in the simulated world, receive consequences, train by return, counterfactual rollouts, actor-critic, or model-based value learning over repair windows, and then fail under targeted social and environmental ablations while acting. Places with function, territory, and social infrastructure should be added only where they create specific control pressure rather than visual detail alone.
