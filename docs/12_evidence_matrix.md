...
|---|---|---|
| Learned control | Partial pass. Report 62 shows recurrent controllers trained without self labels improve embodied control, and reports 69/73 show a designed packet bridge plus a partial no-leak negative rather than stable cross-gate integration. | Online-return or model-based controllers with stronger causal policy-state edits in the full SSRM-3D world. |
...
SSRM-3D persistent pressure layers from perception through dependent care
