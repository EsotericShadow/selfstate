...
| Real social pressure | Partial pass. Return-selected SSRM-3D policies use identity memory, reputation, vulnerability, shared-tool trust, and costly communication; report 89 adds a designed settlement/civilization pressure layer with roles, construction, norms, affect control, and future shocks; report 69 adds a learned social-memory bridge, but report 73 finds integrated social pressure not stable. | Move repeated social interaction, construction, norms, communication, deception, resource conflict, and settlement shocks into learned controllers. |
