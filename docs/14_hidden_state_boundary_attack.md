...
The SSRM-3D dependent care precursor adds a twenty-second boundary: caregiving is not selfhood. Fragile companions, dependent state, identity memory, protection, sharing, repair, teaching, shelter coordination, promises, social trust, and priority arbitration are care-control variables. They become relevant to selfhood only when another persistent vulnerable agent changes the tested agent's own future options, obligations, resources, social access, or restore-time control. No-dependent controls must reject care machinery when there is no dependent agent.

The SSRM-3D irreversible loss precursor adds a twenty-third boundary: loss is not selfhood. Loss memory, value-at-risk estimation, replacement modeling, caution control, preservation, backup, and loss response are future-option control variables. They become relevant to selfhood only when permanent loss changes the tested agent's own future option space, obligations, social access, memory state, or restore-time continuity. Reversible-wear controls must reject loss machinery when all damage can be restored.

The SSRM-3D affective control precursor adds a twenty-fourth boundary: affect is not selfhood or consciousness. Fear, stress, trust, frustration, affiliation, curiosity, and guilt analogues are compact control summaries. They become relevant to selfhood only when they change the tested agent's own attention, risk tolerance, communication, memory salience, repair behavior, information seeking, social access, or restore-time continuity. Calm controls must reject affective machinery when those summaries have no control job.
