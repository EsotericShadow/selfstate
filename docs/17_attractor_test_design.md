...
- SSRM-3D learned-controller testing showing recurrent policy states trained without self labels improve control under pressure and carry decodable self-state.
- SSRM-3D tool-making, social-pressure, social-ecology, social trust/contracts, predator/threat, resource-ecology, injury/disability, development/skill, dependent-care, irreversible-loss, and affective-control testing showing that external tools, identity memory, reputation, commitments, costly signals, threat defenses, caches, restraint, territory, capability compensation, competence history, fragile-companion care, loss response, and compact affective summaries are selected only when they preserve future options.
- SSRM-3D agent-continuity testing showing that restore and fork performance depends on a whole continuity record rather than model weights, memory, or body state alone.
- SSRM-3D learned-integration testing showing a designed packet bridge for local and integrated pressure rows in the seeded canonical run.
- SSRM-3D physics-first benchmarking showing modular physics-derived sensor streams can train recurrent neural decision models without scenario labels, while closed-loop deep RL remains unmet.
...
- pause, restore, rollback, or fork pressure requires a continuity record that binds body, model, memory, social history, commitments, event log, attention, hidden state, tools, goals, and branch identity.
...
- [SSRM-3D learned integration controller report](69_ssrm_3d_learned_integration_controller_report.md)
- [SSRM-3D no-leak integration sweep report](73_ssrm_3d_no_leak_integration_sweep_report.md)
...
- [SSRM-3D physics-first benchmark report](88_ssrm_3d_physics_benchmark_report.md)
