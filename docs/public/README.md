...
- reports now include Report `136`, with roadmap docs through `102`;
- the canonical runner is wired for `116` executable runs;
...
- the newest learned-readiness evidence is Report `136`: a GRU beats reactive/frame scores and preserves the `12h` gate, but all learned-controller agents die by `72h`, knowledge transfer collapses, and readiness ablations are unstable;
