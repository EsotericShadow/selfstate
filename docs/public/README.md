...
- reports now include Report `114`, with roadmap docs through `102`;
- the canonical runner is wired for `94` executable runs;
- the newest evidence step is a bounded randomized-transfer pass: separate learned environmental and social action heads are coordinated across 96h worlds with randomized post-12h crisis timing, order, repetition, and initial pressure, raising held-out coupled-crisis score from `0.000` to `0.706` and resolved rate from `0.067` to `0.967`;
