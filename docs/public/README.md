...
- reports now include Report `112`, with roadmap docs through `102`;
- the canonical runner is wired for `92` executable runs;
- the newest evidence step is a failed diagnostic-memory controller: the recurrent diagnostic head learns the environmental repair label offline at `0.991` accuracy, but validation selects diagnostic bias `0.0` because nonzero bias raises environmental response while killing social response and increasing damage;
