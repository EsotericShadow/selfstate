...
- reports now include Report `105`, with roadmap docs through `102`;
- the canonical runner is wired for `85` executable runs;
- the newest evidence step is a return-selected multi-day maturation controller: validation return chooses a `social_env` pressure router and held-out 72h worlds still mature, but social/culture and environmental ablations still do not reduce total score;
