...
- reports now include Report `137`, with roadmap docs through `102`;
- the canonical runner is wired for `117` executable runs;
...
- the newest learned-readiness evidence is Report `137`: closed-loop recovery relabeling improves the GRU from `0.212` to `0.470` maturation score and final alive from `0.0` to `14.0`, but knowledge transfer is still `0.000` and ablations are not clean;
...
| Environment-readiness maturation | [135](../135_ssrm_3d_environment_readiness_maturation_report.md) | Can the richer sandbox readiness layer be tested headlessly over several days? | Yes as a designed verifier: 72h worlds preserve the 12h shock gate, survive post-gate shocks, build fuel/seed/blueprint/forecast/apprenticeship readiness, and expose channel-specific ablation losses. |
...
| Coupled crisis direct counterfactual policy | [129](../129_ssrm_3d_coupled_crisis_direct_counterfactual_policy_report.md) | Does training the recurrent policy directly from cloned counterfactual window labels solve planner removal? | No: the direct policy trains with `0.615` weighted accuracy, but validation selects direct bias `0.0`; crisis score, resolved rate, and coupled response remain `0.000`. |
| Coupled crisis active consequence optimization | [130](../130_ssrm_3d_coupled_crisis_active_consequence_optimization_report.md) | Does delayed active optimization or a learned consequence-value controller solve planner removal? | Not yet: the value controller improves over return-selected from `0.487` to `0.501` total score, but stays below consequence recovery and reaches only `0.081` coupled response. |
| Coupled crisis MPC sequence optimizer | [131](../131_ssrm_3d_coupled_crisis_mpc_sequence_optimizer_report.md) | Does multi-step cloned-rollout planning recover the coupled repair shape? | Partly: total score rises to `0.683`, crisis score to `0.348`, and coupled response to `0.565`, but the result remains a supplied planning bridge rather than learned transfer. |
| Coupled crisis MPC sequence distillation | [132](../132_ssrm_3d_coupled_crisis_mpc_sequence_distillation_report.md) | Can that MPC behavior be distilled into a recurrent policy without rollout scoring? | No: the student trains on `24633` teacher examples but held-out crisis score, resolved rate, social response, and coupled response collapse to `0.000`. |
| Coupled crisis MPC closed-loop recovery | [133](../133_ssrm_3d_coupled_crisis_mpc_closed_loop_recovery_report.md) | Does relabeling the MPC student's own crisis states make planner removal work? | No: aggregate training grows to `45789` examples and total score improves, but crisis score, resolved rate, and coupled response remain `0.000`. |
| Coupled crisis student-sequence consequence | [134](../134_ssrm_3d_coupled_crisis_student_sequence_consequence_report.md) | Do student-created counterfactual sequence windows make planner removal work? | No: the windows have positive consequence signal, but the student falls below consequence recovery and crisis score, resolved rate, and coupled response remain `0.000`. |
