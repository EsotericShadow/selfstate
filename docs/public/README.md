...
- reports now include Report `106`, with roadmap docs through `102`;
- the canonical runner is wired for `86` executable runs;
- the newest evidence step is a failed coupled social/environment maturation controller: the designed controller resolves post-12h coupled crises, but the learned return-selected GRU preserves generic maturation while failing the coupled-crisis score;
