...
- reports now include Report `131`, with roadmap docs through `102`;
- the canonical runner is wired for `111` executable runs;
- the newest evidence step is a bounded sequence-planning bridge: cloned rollout-scored multi-step commitment raises held-out crisis score to `0.348` and coupled response to `0.565`, but remains `partial_or_failed` because it narrowly misses the strict teacher-transfer threshold and still depends on supplied plan templates plus simulator lookahead;
