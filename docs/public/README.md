...
- reports now include Report `111`, with roadmap docs through `102`;
- the canonical runner is wired for `91` executable runs;
- the newest evidence step is a failed rollout-window controller: plan values train from cloned simulator rollouts, but validation selects plan bias `0.0`, held-out crisis score stays `0.000`, and the strong social/environment ablation dependency still fails;
