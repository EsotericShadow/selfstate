...
- reports now include Report `113`, with roadmap docs through `102`;
- the canonical runner is wired for `93` executable runs;
- the newest evidence step is a bounded positive joint-arbitration controller: separate learned environmental and social action heads are coordinated by validation-selected quotas, raising held-out coupled-crisis score from `0.000` to `0.380` and resolved rate from `0.100` to `0.650`;
