...
- reports now include Report `107`, with roadmap docs through `102`;
- the canonical runner is wired for `87` executable runs;
- the newest evidence step is a failed coupled-crisis repair critic: validation turns the learned repair critic off, so the GRU still preserves generic maturation while failing the coupled-crisis score;
