...
- reports now include Report `121`, with roadmap docs through `102`;
- the canonical runner is wired for `101` executable runs;
- the newest evidence step is a failed recurrent crisis-memory result: carrying hidden state through the active crisis sequence trains across `90` crises, but held-out crisis score, resolved rate, and coupled response still collapse to `0.000`;
