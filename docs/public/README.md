...
- reports now include Report `118`, with roadmap docs through `102`;
- the canonical runner is wired for `98` executable runs;
- the newest evidence step is a failed temporal-return value result: delayed crisis-window labels separate stronger policies from weaker ones, but held-out crisis score still remains `0.000` and the controller does not beat return-selected or fixed-joint baselines;
