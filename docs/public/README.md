...
- reports now include Report `110`, with roadmap docs through `102`;
- the canonical runner is wired for `90` executable runs;
- the newest evidence step is a failed environmental-bottleneck controller: validation selects plan bias `2.75`, some response metrics improve, but held-out crisis score stays `0.000` and the strong social/environment ablation dependency still fails;
