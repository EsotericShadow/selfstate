...
- reports now include Report `120`, with roadmap docs through `102`;
- the canonical runner is wired for `100` executable runs;
- the newest evidence step is a failed actor-critic result: adding a critic/value baseline to sampled crisis-window policy updates trains the value head, but held-out crisis score, resolved rate, and coupled response all collapse to `0.000`;
