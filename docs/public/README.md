...
- reports now include Report `122`, with roadmap docs through `102`;
- the canonical runner is wired for `102` executable runs;
- the newest evidence step is a failed two-channel process-pressure result: the policy trains across `91` crises, but held-out crisis score, resolved rate, and coupled response still collapse to `0.000`;
