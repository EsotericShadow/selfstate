...
- reports now include Report `133`, with roadmap docs through `102`;
- the canonical runner is wired for `113` executable runs;
- the newest evidence step is another failed learned-transfer boundary: student-state relabeling raises total score from `0.468` to `0.506`, but after simulator rollout scoring is removed, held-out crisis score, resolved rate, and coupled response still fall to `0.000`;
