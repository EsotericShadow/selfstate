...
- reports now include Report `132`, with roadmap docs through `102`;
- the canonical runner is wired for `112` executable runs;
- the newest evidence step is a failed learned-transfer boundary: the recurrent student fits MPC teacher traces with `0.596` accuracy, but after simulator rollout scoring is removed, held-out crisis score, resolved rate, social response, and coupled response all fall to `0.000`;
