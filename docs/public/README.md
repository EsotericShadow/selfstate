...
- reports now include Report `116`, with roadmap docs through `102`;
- the canonical runner is wired for `96` executable runs;
- the newest evidence step is a partial negative policy/value-allocation result: a value selector improves tune-world allocator choice, but the selected allocator transfers worse than the seed/fixed allocator on held-out 96h randomized coupled-crisis worlds;
