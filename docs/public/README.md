...
- reports now include Report `117`, with roadmap docs through `102`;
- the canonical runner is wired for `97` executable runs;
- the newest evidence step is a failed active state/action value result: a value head trains on active-crisis examples, but held-out crisis score remains `0.000` and the controller does not beat return-selected or fixed-joint baselines;
