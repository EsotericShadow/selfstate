...
- reports now include Report `109`, with roadmap docs through `102`;
- the canonical runner is wired for `89` executable runs;
- the newest evidence step is a partial coupled-crisis sequence-outcome controller: validation selects plan bias `4.0` and held-out crisis repair improves, but the strong social/environment ablation dependency still fails;
