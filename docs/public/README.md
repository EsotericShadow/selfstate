...
- reports now include Report `125`, with roadmap docs through `102`;
- the canonical runner is wired for `105` executable runs;
- the newest evidence step is a failed closed-loop recovery boundary: DAgger-style student-state relabeling expands training to `74756` examples, but after planner removal held-out crisis score, resolved rate, and coupled response remain `0.000`;
