...
- reports now include Report `126`, with roadmap docs through `102`;
- the canonical runner is wired for `106` executable runs;
- the newest evidence step is a partial consequence-recovery boundary: completed-crisis weighting raises held-out crisis score to `0.028` and coupled response to `0.355`, but the policy remains far below the planner and fixed-joint baselines;
