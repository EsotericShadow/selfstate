...
- reports now include Report `115`, with roadmap docs through `102`;
- the canonical runner is wired for `95` executable runs;
- the newest evidence step is a partial adaptive-allocation result: return search over a compact allocator improves 96h randomized coupled-crisis repair over return-selected and fixed-joint baselines, but fails the stronger non-fixed-transfer gate;
