...
- reports now include Report `119`, with roadmap docs through `102`;
- the canonical runner is wired for `99` executable runs;
- the newest evidence step is a failed active-policy result: sampled crisis-window policy updates improve coupled response over the return-selected controller, but held-out crisis score still remains `0.000` and fixed-joint coordination is much stronger;
