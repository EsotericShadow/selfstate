...
- reports now include Report `108`, with roadmap docs through `102`;
- the canonical runner is wired for `88` executable runs;
- the newest evidence step is a failed coupled-crisis outcome-value controller: validation selects a nonzero value bias, but held-out crisis repair gets worse than the return-selected GRU;
