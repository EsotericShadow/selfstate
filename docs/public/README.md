...
- reports now include Report `124`, with roadmap docs through `102`;
- the canonical runner is wired for `104` executable runs;
- the newest evidence step is a failed planner-distillation boundary: a recurrent policy fits minimum-channel planner traces with `0.986` imitation accuracy, but after planner removal held-out crisis score remains `0.000`;
