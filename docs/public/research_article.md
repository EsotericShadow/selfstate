...
That sandbox is a prototype for the next benchmark, not a new proof result. The important next step is distillation: train an active learned policy from successful minimum-channel trajectories, then remove the engineered planner at evaluation while keeping held-out worlds and targeted ablations. Reports 112 through 123 make the lesson sharper: an offline-correct diagnostic head is not enough by itself, a controller that keeps environmental and social repair active together can cross the coupled-crisis gate, that bridge survives randomized post-gate timing and initial pressure, a return-searched allocator helps but remains too weak to retire structured coordination, a value selector over allocator policies can still overfit tune worlds, single-step active state/action value labels still fail delayed repair, passive temporal-return labels still do not produce robust active repair policy, lightweight sampled policy gradients improve response behavior without recovering nonzero crisis score, a shallow actor-critic value baseline can train tightly while worsening held-out crisis action, recurrent crisis memory still fails if the learned policy drops one repair channel, naive two-channel process pressure can still collapse into a one-channel shortcut, and dynamic weakest-channel planning can recover coupled repair when wrapped around learned action heads.
