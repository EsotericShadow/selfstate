#!/usr/bin/env python3
"""Environmental-bottleneck sequence planner for SSRM-3D coupled crises.

Report 109 showed that sequence-window control can improve coupled crisis
repair, but the environment ablation remained too weak. This benchmark makes
the environmental side non-substitutable: each crisis has one primary
environmental repair class, and generic environmental activity does not count as
repair.
...
PREFIX = ARTIFACT_DIR / "ssrm_3d_coupled_crisis_environment_bottleneck"
...
class VerdictRow:
    ...
    environment_bottleneck_total_score: float
    ...
    environment_bottleneck_crisis_score: float
    return_selected_crisis_score: float
    environment_bottleneck_resolved_rate: float
    return_selected_resolved_rate: float
    environment_bottleneck_coupled_response: float
    ...
    supports_environment_bottleneck_selection: bool
    supports_social_environment_dependency: bool
    verdict: str


class EnvironmentBottleneckPlanNet(nn.Module):
...
def sequence_plan_value(
    ...
    remaining = max(0.0, active.end - (active.start + active.steps * dt))
    window = min(plan.horizon_hours, max(dt, remaining))
    env_gap = 1.0 - env_fraction
    social_gap = 1.0 - social_fraction
    primary_env = primary_env_action(active)
    env_fit = symptom_fit(features, primary_env) * 0.76 + action_skill(features, primary_env) * 0.24
    social_fit = best_group_fit(features, active.profile.social_actions)
    current_env_pressure = max(env_context, recent_env * 0.75)
    current_social_pressure = max(social_context, recent_social * 0.75)

    env_effort = clamp(
        plan.env_weight * (0.58 + env_fit * 0.42)
        + plan.infrastructure_weight * 0.12
        + plan.sustain_weight * 0.08
        + current_env_pressure * 0.20
    )
    social_effort = clamp(
        plan.social_weight * (0.68 + social_fit * 0.24)
        + plan.teaching_weight * 0.15
        + current_social_pressure * 0.18
    )

    # Penalize wasting the whole next window on a side that is already being
    # serviced while the opposite side is starving. The learned model sees only
    # symptoms and response context; this target supplies the sequence credit.
    env_effort *= 0.65 + env_gap * 0.35 + max(0.0, 0.22 - current_env_pressure)
    social_effort *= 0.65 + social_gap * 0.35 + max(0.0, 0.20 - current_social_pressure)
    next_env_fraction = min(1.0, (active.env_progress + env_effort * window * 1.70) / active.profile.env_need)
    next_social_fraction = min(1.0, (active.social_progress + social_effort * window * 1.65) / active.profile.social_need)
    next_coupled = min(next_env_fraction, next_social_fraction)
    coupled_gain = max(0.0, next_coupled - current_coupled)

    balanced_gain = min(
        max(0.0, next_env_fraction - env_fraction),
        max(0.0, next_social_fraction - social_fraction),
    )
    resolution_bonus = 0.55 if next_coupled >= 0.92 else 0.0
    missing_side_bonus = (
        env_gap * max(0.0, 0.32 - current_env_pressure) * plan.env_weight
        + social_gap * max(0.0, 0.30 - current_social_pressure) * plan.social_weight
    )
    unresolved_after = 1.0 - next_coupled
    overfocus_cost = abs(plan.env_weight - plan.social_weight) * 0.06 if env_gap > 0.25 and social_gap > 0.25 else 0.0
    fatigue_cost = plan.sustain_weight * 0.05 if features[FEATURE["energy"]] > 0.55 else -plan.sustain_weight * 0.10
    value = (
        coupled_gain * 8.5
        + balanced_gain * 4.2
        + resolution_bonus
        + missing_side_bonus
        + env_fit * plan.env_weight * 0.20
        + social_fit * plan.social_weight * 0.22
        + fatigue_cost
        - unresolved_after * 0.20
        - overfocus_cost
    )
    return max(-0.60, min(1.85, value))
    ...
    for seed in cfg.train_seeds:
        ...
        while world.time < cfg.hours - 1e-9 and len(examples) < cfg.max_plan_examples:
            ...
            def selector(
                ...
                if active is not None and current_world.time >= 12.0 and len(examples) < cfg.max_plan_examples:
                    if rng.random() < 0.32:
                        ...
                        for env_context, social_context, recent_env, recent_social in rng.sample(contexts, k=2):
                            for plan in PLAN_TEMPLATES:
                                ...
                                targets.append(sequence_plan_value(plan, features, active, dt, env_context, social_context, recent_env, recent_social))
...
def train_plan_model(cfg: Config, device: torch.device) -> Tuple[EnvironmentBottleneckPlanNet, PlanTrainingRow]:
    examples, targets = collect_plan_examples(cfg)
    if not examples:
        raise RuntimeError("no environment-bottleneck examples collected")
    ...
    model = EnvironmentBottleneckPlanNet(PLAN_INPUT_SIZE, cfg.plan_hidden_size).to(device)
...
def environment_bottleneck_bias(
    features: Sequence[float],
    plan_model: EnvironmentBottleneckPlanNet,
    plan_bias: float,
    device: torch.device,
    dtype: torch.dtype,
    ablation: str,
    ...
    env_missing = max(0.0, 0.36 - env_pressure)
    social_missing = max(0.0, 0.34 - social_pressure)
    ...
    for action in base.ACTIONS:
        ...
        if action in ENV_RESPONSE_ACTIONS:
            ...
            if action == best_env_action and best_env_score >= 0.13:
                score += (plan_mix["env"] + env_missing + social_overserved) * (0.82 + best_env_score * 1.70)
            elif action in {"collect_water", "harvest_food"}:
                score -= (plan_mix["env"] + env_missing) * 0.18
        if action in SOCIAL_RESPONSE_ACTIONS:
            ...
            if env_missing > 0.16 and best_env_score >= 0.13:
                social_score -= env_missing * (0.44 + best_env_score * 0.42)
...
def run_episode(
    ...
    device: torch.device,
    router: report105.PressureRouter,
    plan_model: Optional[EnvironmentBottleneckPlanNet] = None,
    plan_bias: float = 0.0,
    ablation: str = "none",
    trace: bool = False,
) -> Tuple[coupled.EvalRow, Trace, coupled.CrisisTracker]:
    if controller == "environment_bottleneck_gru" and (model is None or plan_model is None):
        raise ValueError("environment_bottleneck_gru requires both base model and environment bottleneck model")
...
def selection_objective(rows: Sequence[coupled.EvalRow]) -> Tuple[float, float, float, float, float, float]:
    ...
    objective = total + crisis * 0.18 + resolved * 0.12 + coupled_response * 0.11 - damage * 0.26
...
def select_plan_bias(
    ...
    for bias in cfg.plan_bias_candidates:
        eval_rows = [
            run_episode(seed, cfg, "environment_bottleneck_gru", model, device, router, plan_model, bias)[0]
...
def ablations_from_summary(summary: Sequence[coupled.SummaryRow]) -> List[coupled.AblationRow]:
    base_row = coupled.row_lookup(summary, "environment_bottleneck_gru", "none")
    rows: List[coupled.AblationRow] = []
    for ablation in ("body", "infrastructure", "tools", "social_culture", "environment", "previous_action"):
        row = coupled.row_lookup(summary, "environment_bottleneck_gru", ablation)
        rows.append(
            coupled.AblationRow(
                ablation=ablation,
...
def verdict_from_summary(
    ...
    outcome = coupled.row_lookup(summary, "environment_bottleneck_gru", "none")
    ...
    return VerdictRow(
        ...
        environment_bottleneck_total_score=outcome.mean_total_score,
        ...
        environment_bottleneck_crisis_score=outcome.mean_crisis_score,
        return_selected_crisis_score=returned.mean_crisis_score,
        environment_bottleneck_resolved_rate=outcome.mean_resolved_rate,
        return_selected_resolved_rate=returned.mean_resolved_rate,
        environment_bottleneck_coupled_response=outcome.mean_coupled_response_rate,
        ...
        supports_environment_bottleneck_selection=supports_selection,
...
def run_benchmark(cfg: Config) -> dict[str, object]:
    ...
    trace = Trace(seed=cfg.trace_seed, condition=f"environment_bottleneck_gru:{selected_router.name}:bias_{selected_bias:g}:none")
    ...
    for seed in cfg.eval_seeds:
        for controller, model, router, plan_net, bias in (
            ...
            ("environment_bottleneck_gru", models["gru"], selected_router, plan_model, selected_bias),
        ):
            row, maybe_trace, tracker = run_episode(
                ...
                plan_net,
                bias,
                trace=(seed == cfg.trace_seed and controller == "environment_bottleneck_gru"),
            )
        ...
        for ablation in ("body", "infrastructure", "tools", "social_culture", "environment", "previous_action"):
            ...
            eval_rows.append(row)
            crisis_logs[f"{seed}:environment_bottleneck_gru:{ablation}"] = tracker.response_log
...
def main() -> int:
    ...
    # A failed environment-bottleneck verdict is evidence, not a process failure.
    return 0
