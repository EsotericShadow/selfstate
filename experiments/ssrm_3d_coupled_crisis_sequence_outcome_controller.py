#!/usr/bin/env python3
"""Sequence-outcome planner for coupled SSRM-3D maturation crises.

Report 107 showed that a supervised repair-label critic is rejected by
validation return, and Report 108 showed that per-step outcome-value reranking
can overfit validation while worsening held-out crises. This benchmark moves the
learning target up one level: a small plan-value model scores short repair
windows, not isolated actions.
...
PREFIX = ARTIFACT_DIR / "ssrm_3d_coupled_crisis_sequence_outcome"
...
class VerdictRow:
    ...
    sequence_outcome_total_score: float
    ...
    sequence_outcome_crisis_score: float
    return_selected_crisis_score: float
    sequence_outcome_resolved_rate: float
    return_selected_resolved_rate: float
    sequence_outcome_coupled_response: float
    ...
    supports_sequence_outcome_selection: bool
    supports_social_environment_dependency: bool
    verdict: str


class SequencePlanNet(nn.Module):
...
def sequence_plan_value(
    ...
    env_fit = best_group_fit(features, active.profile.env_actions)
    ...
    env_effort = clamp(
        plan.env_weight * (0.66 + env_fit * 0.28)
    ...
    next_env_fraction = min(1.0, (active.env_progress + env_effort * window * 1.55) / active.profile.env_need)
...
def train_plan_model(cfg: Config, device: torch.device) -> Tuple[SequencePlanNet, PlanTrainingRow]:
    examples, targets = collect_plan_examples(cfg)
    if not examples:
        raise RuntimeError("no sequence-outcome examples collected")
    ...
    model = SequencePlanNet(PLAN_INPUT_SIZE, cfg.plan_hidden_size).to(device)
...
def sequence_outcome_bias(
    features: Sequence[float],
    plan_model: SequencePlanNet,
    plan_bias: float,
    device: torch.device,
    dtype: torch.dtype,
    ablation: str,
    ...
    for action in base.ACTIONS:
        ...
        if action in ENV_RESPONSE_ACTIONS:
            score += plan_mix["env"] * (0.42 + fit * 0.45 + skill * 0.12) * (0.86 + env_missing * 1.8)
            score += env_missing * 0.70
            score += social_overserved * (1.25 + fit * 0.36 + skill * 0.12)
        ...
        if action in SOCIAL_RESPONSE_ACTIONS:
            ...
            social_score -= social_overserved * 0.55
...
def run_episode(
    ...
    plan_model: Optional[SequencePlanNet] = None,
    plan_bias: float = 0.0,
    ablation: str = "none",
    trace: bool = False,
) -> Tuple[coupled.EvalRow, Trace, coupled.CrisisTracker]:
    if controller != "sequence_outcome_gru":
        return coupled.run_episode(seed, cfg, controller, model, device, router, ablation=ablation, trace=trace)
    if model is None or plan_model is None:
        raise ValueError("sequence_outcome_gru requires both base model and sequence outcome model")
    ...
    while world.time < cfg.hours - 1e-9:
        ...
        def selector(
            ...
            with torch.no_grad():
                ...
                plan_logits = sequence_outcome_bias(
                    features,
                    plan_model,
                    plan_bias,
                    device,
...
def select_plan_bias(
    ...
    for bias in cfg.plan_bias_candidates:
        eval_rows = [
            run_episode(seed, cfg, "sequence_outcome_gru", model, device, router, plan_model, bias)[0]
...
def ablations_from_summary(summary: Sequence[coupled.SummaryRow]) -> List[coupled.AblationRow]:
    base_row = coupled.row_lookup(summary, "sequence_outcome_gru", "none")
    rows: List[coupled.AblationRow] = []
    for ablation in ("body", "infrastructure", "tools", "social_culture", "environment", "previous_action"):
        row = coupled.row_lookup(summary, "sequence_outcome_gru", ablation)
        rows.append(
            coupled.AblationRow(
                ablation=ablation,
...
def verdict_from_summary(
    ...
    outcome = coupled.row_lookup(summary, "sequence_outcome_gru", "none")
    ...
    return VerdictRow(
        ...
        sequence_outcome_total_score=outcome.mean_total_score,
        ...
        sequence_outcome_crisis_score=outcome.mean_crisis_score,
        return_selected_crisis_score=returned.mean_crisis_score,
        sequence_outcome_resolved_rate=outcome.mean_resolved_rate,
        return_selected_resolved_rate=returned.mean_resolved_rate,
        sequence_outcome_coupled_response=outcome.mean_coupled_response_rate,
        ...
        supports_sequence_outcome_selection=supports_selection,
...
def run_benchmark(cfg: Config) -> dict[str, object]:
    ...
    trace = Trace(seed=cfg.trace_seed, condition=f"sequence_outcome_gru:{selected_router.name}:bias_{selected_bias:g}:none")
    ...
    for seed in cfg.eval_seeds:
        for controller, model, router, plan_net, bias in (
            ...
            ("sequence_outcome_gru", models["gru"], selected_router, plan_model, selected_bias),
        ):
            row, maybe_trace, tracker = run_episode(
                ...
                plan_net,
                bias,
                trace=(seed == cfg.trace_seed and controller == "sequence_outcome_gru"),
            )
        ...
        for ablation in ("body", "infrastructure", "tools", "social_culture", "environment", "previous_action"):
            ...
            eval_rows.append(row)
            crisis_logs[f"{seed}:sequence_outcome_gru:{ablation}"] = tracker.response_log
    ...
    write_js(Path(f"{PREFIX}_results.js"), "SSRM_3D_COUPLED_CRISIS_SEQUENCE_OUTCOME_RESULTS", payload)
    write_js(Path(f"{PREFIX}_trace.js"), "SSRM_3D_COUPLED_CRISIS_SEQUENCE_OUTCOME_TRACE", asdict(trace))
    return payload
...
def main() -> int:
    ...
    # A failed sequence-outcome verdict is evidence, not a process failure.
    return 0
