...
let world = JSON.parse(localStorage.getItem(STATE_KEY) || JSON.stringify({
  ...
  gamePrototypeMovementRoute: null,
  gamePrototypeResidentEncounter: null,
  gamePrototypeObjectInteraction: null,
...
function leaveAndReturnLater(daysAway = 3) {
  ...
  const beforeVisualRows = world.gamePrototypeProjects && world.gamePrototypeProjects.visualLedger ? world.gamePrototypeProjects.visualLedger.length : 0;
  ...
  const afterVisualRows = world.gamePrototypeProjects && world.gamePrototypeProjects.visualLedger ? world.gamePrototypeProjects.visualLedger.length : 0;
  ...
  const receipt = {
    ...
    project_visual_rows_before: beforeVisualRows,
    project_visual_rows_after: afterVisualRows,
    project_visual_rows_added: Math.max(0, afterVisualRows - beforeVisualRows),
    latest_project_visual_id: latestProjectVisual ? latestProjectVisual.visual_id : 'none',
    latest_project_visual_cue: latestProjectVisual ? latestProjectVisual.canvas_cue : 'none',
    replay_before: replayBefore,
...
function runFirstPlayablePrototypeLoop() {
  ...
  return log('runFirstPlayablePrototypeLoop', {
    ...
    movementRouteReady: world.gamePrototypeMovementRoute ? world.gamePrototypeMovementRoute.acceptanceReady === true : false,
    residentEncounterReady: world.gamePrototypeResidentEncounter ? world.gamePrototypeResidentEncounter.acceptanceReady === true : false,
    objectInteractionReady: world.gamePrototypeObjectInteraction ? world.gamePrototypeObjectInteraction.acceptanceReady === true : false,
...
function ensureNormalPlayActionRail() {
  ...
  if (!world.gamePrototypeActionRail) {
    world.gamePrototypeActionRail = {
      ...
      receipt: null,
      acceptanceReady: false,
      verbs: ['look', 'move', 'ask', 'talk', 'objects', 'support', 'wait', 'return', 'save'],
  ...
  const preferredRailVerbs = ['look', 'move', 'ask', 'talk', 'objects', 'handling', 'support', 'wait', 'return', 'save'];
...
function runNormalPlayAction(verb) {
  ...
  else if (verb === 'talk') receipt = runPlayerResidentEncounterLoop();
  else if (verb === 'objects') receipt = runPlayerObjectInteractionLoop();
  else if (verb === 'handling') receipt = runResidentMaterialManipulationStep('resident_choice');
...
function runNormalPlayMove() { return runNormalPlayAction('move'); }
function runNormalPlayAsk() { return runNormalPlayAction('ask'); }
function runNormalPlayTalk() { return runNormalPlayAction('talk'); }
function runNormalPlayObjects() { return runNormalPlayAction('objects'); }
function runNormalPlayHandling() { return runNormalPlayAction('handling'); }
...
function formatNormalPlayActionRail() {
  ...
  const actionRows = rail.actionLedger.slice(-8).map(row => `${row.action_id}: ${row.label} -> ${row.underlying_action}; proposal=${row.proposal_id}; practice=${row.practice_id}; handling=${row.manipulation_id || 'none'}; save=${row.save_slot_id}`);
  return [
...
function ensureReturnJournal() {
  ensureGamePrototype();
  if (!world.gamePrototypeReturnJournal) {
    world.gamePrototypeReturnJournal = {
      milestone: 'return-journal',
      runCount: 0,
      journalLedger: [],
      snapshotLedger: [],
      receipt: null,
      acceptanceReady: false,
      playerFacing: true,
      forwardReturnVisible: true,
      saveRestoreVisible: true,
      noDirectReset: true,
      noHiddenLawNormalView: true,
      boundary: 'player-facing return journal; shows away-time consequences and save-slot restoration without hiding source history',
    };
  }
  return world.gamePrototypeReturnJournal;
}

function returnJournalSnapshot(label) {
  const saves = world.gamePrototypeSaves || ensurePrototypeSaves();
  const returnLater = world.gamePrototypeReturnLater || null;
  const latestForward = returnLater && returnLater.returnLedger.length ? returnLater.returnLedger[returnLater.returnLedger.length - 1] : null;
  const latestRestore = saves.returnLog.length ? saves.returnLog[saves.returnLog.length - 1] : null;
  ...
  return {
    ...
    project_visual_rows: projects && projects.visualLedger ? projects.visualLedger.length : 0,
    latest_project_visual_id: latestProjectVisual ? latestProjectVisual.visual_id : 'none',
    latest_project_visual_cue: latestProjectVisual ? latestProjectVisual.canvas_cue : 'none',
    ...
    save_returns: saves.returnLog.length,
    forward_returns: returnLater ? returnLater.returnLedger.length : 0,
    latest_forward_return: latestForward ? latestForward.return_id : 'none',
    latest_forward_days: latestForward ? latestForward.days_away : 0,
    latest_forward_remembered: latestForward ? latestForward.residents_who_remembered : [],
    latest_forward_restored_old_state: latestForward ? latestForward.restored_old_state === true : false,
    latest_forward_project_visual_rows_added: latestForward ? Number(latestForward.project_visual_rows_added || 0) : 0,
    latest_forward_project_visual_id: latestForward ? latestForward.latest_project_visual_id || 'none' : 'none',
...
function updateReturnJournalAcceptance() {
  const journal = ensureReturnJournal();
  ...
  journal.acceptanceReady = Boolean(
    journal.journalLedger.length > 0 &&
    journal.snapshotLedger.length >= 3 &&
    latest.forward_returns > 0 &&
    latest.save_slots > 0 &&
    ...
    journal.noDirectReset === true &&
    journal.noHiddenLawNormalView === true
  );
  journal.receipt = {
    receipt_id: `RTJ-${String(journal.runCount).padStart(2, '0')}`,
    acceptance_ready: journal.acceptanceReady,
    journal_rows: journal.journalLedger.length,
    snapshots: journal.snapshotLedger.length,
    forward_returns: latest.forward_returns,
    save_slots: latest.save_slots,
  ...
  return journal.acceptanceReady;
}

function pushReturnJournalSnapshot(label) {
  const journal = ensureReturnJournal();
  const snapshot = returnJournalSnapshot(label);
  journal.snapshotLedger.push({
    snapshot_id: `RTJS-${String(journal.snapshotLedger.length + 1).padStart(2, '0')}`,
    tick: world.tick,
    snapshot,
  });
  if (journal.snapshotLedger.length > 20) journal.snapshotLedger.shift();
  return snapshot;
}

function cloneReturnJournal(journal) {
  return {
    ...journal,
    journalLedger: journal.journalLedger.slice(),
    snapshotLedger: journal.snapshotLedger.slice(),
    receipt: journal.receipt ? { ...journal.receipt } : null,
  };
}

function restoreReturnJournalAfterWorldSwap(preserved) {
  if (!world.gamePrototypeReturnJournal || world.gamePrototypeReturnJournal.journalLedger.length < preserved.journalLedger.length || world.gamePrototypeReturnJournal.snapshotLedger.length < preserved.snapshotLedger.length || world.gamePrototypeReturnJournal.runCount < preserved.runCount) {
    world.gamePrototypeReturnJournal = preserved;
  }
  return ensureReturnJournal();
}

function runReturnJournalLoop() {
  const journal = ensureReturnJournal();
  journal.runCount += 1;
  if (!world.gamePrototypeWorksite || !world.gamePrototypeWorksite.acceptanceReady) runResidentWorksiteLoop();
  if (!world.gamePrototypePlayerMode || !world.gamePrototypePlayerMode.enabled) enterPlayerMode();
  const before = pushReturnJournalSnapshot('before-save-and-away-time');
  const slotResult = savePrototypeSlot('return journal before absence');
  const slotId = slotResult && slotResult.payload ? slotResult.payload.slotId : before.active_slot;
  const awayResult = leaveAndReturnLater(2);
  const afterAway = pushReturnJournalSnapshot('after-forward-return');
  const preservedJournal = cloneReturnJournal(journal);
  const restoreResult = returnPrototypeSlot();
  const activeJournal = restoreReturnJournalAfterWorldSwap(preservedJournal);
  const afterRestore = pushReturnJournalSnapshot('after-save-slot-return');
  const row = {
    journal_id: `RTJ-${String(activeJournal.journalLedger.length + 1).padStart(2, '0')}`,
    tick: world.tick,
    slot_id: slotId || 'none',
    forward_return_id: afterAway.latest_forward_return,
    days_away: afterAway.latest_forward_days,
    remembered_residents: afterAway.latest_forward_remembered,
    resource_total_before: before.resource_total,
    resource_total_after_away: afterAway.resource_total,
    ...
    project_visual_rows_before: before.project_visual_rows,
    project_visual_rows_after_away: afterAway.project_visual_rows,
    project_visual_rows_after_restore: afterRestore.project_visual_rows,
    project_visual_rows_added_away: Math.max(0, afterAway.project_visual_rows - before.project_visual_rows),
    latest_project_visual_after_away: afterAway.latest_project_visual_id,
    ...
    forward_return_visible: true,
    save_restore_visible: restoreResult && restoreResult.payload ? restoreResult.payload.restored === true : afterRestore.save_returns > before.save_returns,
    direct_reset: false,
    hidden_law_normal_view: false,
  };
  activeJournal.journalLedger.push(row);
  if (activeJournal.journalLedger.length > 20) activeJournal.journalLedger.shift();
  recordRealityConstraint('return_journal', {
    ...
    workCost: 0,
    toolWear: 0,
    maintenanceObligation: 'preserve return source history in normal view',
    unintendedConsequence: 'save-slot restoration is distinguished from forward offscreen continuity',
    hiddenLawInvolved: 'none in normal view',
    conservationCheck: true
  });
  updateReturnJournalAcceptance();
  const updatedJournal = ensureReturnJournal();
  recordPrototypeMilestone('return-journal', {
    ready: updatedJournal.acceptanceReady,
    rows: updatedJournal.journalLedger.length,
    slot: row.slot_id,
    forward: row.forward_return_id,
  });
  return log('runReturnJournalLoop', { ready: updatedJournal.acceptanceReady, rows: updatedJournal.journalLedger.length, forwardReturn: row.forward_return_id, slotId: row.slot_id, saveReturns: afterRestore.save_returns, projectVisualRows: afterRestore.project_visual_rows, latestProjectVisualId: afterRestore.latest_project_visual_id });
}
...
function formatReturnJournal() {
  ...
  return [
    ...
    ...(rows.length ? rows : ['No return journal rows yet.'])
  ].join('\n');
}

function ensurePlayerMovementRoute() {
  ensureGamePrototype();
  if (!world.gamePrototypeMovementRoute) {
    world.gamePrototypeMovementRoute = {
      milestone: 'player-movement-route',
      runCount: 0,
      routeLedger: [],
      snapshotLedger: [],
      receipt: null,
      ...
      noHiddenLawNormalView: true,
      noTechTreeUnlock: true,
      boundary: 'player-facing movement through village space; movement updates room, zone, and nearby affordances without commanding residents',
    };
  }
  ['routeLedger', 'snapshotLedger'].forEach(key => {
    if (!Array.isArray(world.gamePrototypeMovementRoute[key])) world.gamePrototypeMovementRoute[key] = [];
  });
  return world.gamePrototypeMovementRoute;
}

function movementRouteSnapshot(label) {
  const zone = locationZoneForAvatar();
  const plan = nearbyActionPlan();
  const terrain = world.gamePrototypeTerrain && world.gamePrototypeTerrain.terrainLedger && world.gamePrototypeTerrain.terrainLedger.length ? world.gamePrototypeTerrain.terrainLedger[world.gamePrototypeTerrain.terrainLedger.length - 1] : null;
  ...
  return {
    ...
    tick: world.tick,
    room: world.avatar.room,
    x: Number(world.avatar.x || 0),
    y: Number(world.avatar.y || 0),
    zone_id: zone.zone_id,
    zone_label: zone.label,
    nearby_action: plan.action,
    nearby_reason: plan.why,
    selected_resident: world.selected,
    terrain_step: terrain ? terrain.step_id : 'none',
    terrain_weak_cells: terrain ? terrain.weak_cells : 0,
    selected_body_position: body ? { ...body.position3d } : null,
    selected_body_fatigue: body ? Number(body.fatigue || 0) : null,
    player_facing: true,
    avatar_direct_command: false,
    hidden_law_normal_view: false,
    tech_tree_unlock: false,
...
function nextMovementDirection() {
  const route = ensurePlayerMovementRoute();
  const directions = ['east', 'south', 'west', 'north'];
  return directions[route.runCount % directions.length];
}

function runMovementDirection(direction) {
  if (direction === 'north') return moveNorth();
  if (direction === 'south') return moveSouth();
  if (direction === 'west') return moveWest();
  return moveEast();
}

function updatePlayerMovementRouteAcceptance() {
  const route = ensurePlayerMovementRoute();
  route.acceptanceReady = Boolean(
    route.routeLedger.length > 0 &&
    route.snapshotLedger.length > 0 &&
    route.routeLedger.every(row => row.player_facing === true && row.avatar_direct_command === false && row.hidden_law_normal_view === false && row.tech_tree_unlock === false && row.no_teleport === true && row.distance > 0 && row.distance <= 60) &&
    route.noTeleport === true &&
    route.noDirectCommand === true &&
    route.noHiddenLawNormalView === true &&
    route.noTechTreeUnlock === true
  );
  route.receipt = {
    receipt_id: `PMR-${String(route.runCount).padStart(2, '0')}`,
    acceptance_ready: route.acceptanceReady,
    route_rows: route.routeLedger.length,
    snapshots: route.snapshotLedger.length,
    latest_zone: route.routeLedger.length ? route.routeLedger[route.routeLedger.length - 1].after.zone_label : 'none',
    boundary: route.boundary,
  };
  return route.acceptanceReady;
}

function runPlayerMovementRouteLoop() {
  const route = ensurePlayerMovementRoute();
  if (!world.entered) runPrototypeOpening();
  route.runCount += 1;
  const direction = nextMovementDirection();
  const before = movementRouteSnapshot('before-move');
  const moveReceipt = runMovementDirection(direction);
  const after = movementRouteSnapshot('after-move');
  const distance = Number(Math.hypot(after.x - before.x, after.y - before.y).toFixed(3));
  const row = {
    route_id: `PMR-${String(route.routeLedger.length + 1).padStart(2, '0')}`,
    tick: world.tick,
    direction,
    before,
    after,
    distance,
    movement_event: moveReceipt && moveReceipt.event ? moveReceipt.event : `move-${direction}`,
    step: moveReceipt && moveReceipt.payload ? moveReceipt.payload.step : distance,
    nearby_action_after: after.nearby_action,
    nearby_reason_after: after.nearby_reason,
    player_facing: true,
    avatar_direct_command: false,
    hidden_law_normal_view: false,
    tech_tree_unlock: false,
    no_teleport: distance > 0 && distance <= 60,
  };
  route.routeLedger.push(row);
  if (route.routeLedger.length > 40) route.routeLedger.shift();
  route.snapshotLedger.push({ snapshot_id: `PMRS-${String(route.snapshotLedger.length + 1).padStart(2, '0')}`, tick: world.tick, before, after });
  if (route.snapshotLedger.length > 40) route.snapshotLedger.shift();
  updatePlayerMovementRouteAcceptance();
  recordRealityConstraint('player_movement_route', {
    resident: world.selected,
    sourceBeliefId: row.route_id,
    materials: ['avatar_body_position', 'village_space', after.zone_id],
    publicObservation: `avatar moved ${direction} from ${before.zone_label} to ${after.zone_label}`,
    residentInterpretation: `${after.zone_label} now offers ${after.nearby_action}`,
    materialTransformation: `avatar position changed by ${distance} unit(s); no resource or structure spawned`,
    timeCost: 1,
    workCost: 1,
    toolWear: 0,
    maintenanceObligation: `nearby action available: ${after.nearby_action}`,
    unintendedConsequence: 'movement changes which resident/world affordances are nearby',
    hiddenLawInvolved: 'none in normal view',
    conservationCheck: true
  });
  recordPrototypeMilestone('player-movement-route', {
    ready: route.acceptanceReady,
    rows: route.routeLedger.length,
    direction,
    zone: after.zone_id,
  });
  return log('runPlayerMovementRouteLoop', { ready: route.acceptanceReady, rows: route.routeLedger.length, direction, zone: after.zone_id, distance, nearbyAction: after.nearby_action });
}

function formatPlayerMovementRoute() {
  const route = world.gamePrototypeMovementRoute || ensurePlayerMovementRoute();
  const latest = route.snapshotLedger.length ? route.snapshotLedger[route.snapshotLedger.length - 1].after : movementRouteSnapshot('current');
  const rows = route.routeLedger.slice(-6).map(row => `${row.route_id}: ${row.direction}; ${row.before.zone_label}->${row.after.zone_label}; dist=${row.distance}; nearby=${row.nearby_action_after}`);
  return [
    `Acceptance ready: ${route.acceptanceReady ? 'yes' : 'no'}`,
    `Rows: ${route.routeLedger.length} / snapshots=${route.snapshotLedger.length}`,
    `Boundary: ${route.boundary}`,
    `Current position: (${latest.x}, ${latest.y}) room=${latest.room}; zone=${latest.zone_label}`,
    `Nearby affordance: ${latest.nearby_action} because ${latest.nearby_reason}`,
    `Terrain/body: terrain=${latest.terrain_step}; weak=${latest.terrain_weak_cells}; selected fatigue=${latest.selected_body_fatigue === null ? 'n/a' : latest.selected_body_fatigue}`,
    `No teleport: ${route.noTeleport ? 'yes' : 'no'} / no direct command: ${route.noDirectCommand ? 'yes' : 'no'} / no hidden law normal view: ${route.noHiddenLawNormalView ? 'yes' : 'no'}`,
    'Recent movement rows:',
    ...(rows.length ? rows : ['No player movement rows yet.'])
  ].join('\n');
}

function ensurePlayerResidentEncounter() {
  ensureGamePrototype();
  if (!world.gamePrototypeResidentEncounter) {
    world.gamePrototypeResidentEncounter = {
      milestone: 'player-resident-encounter',
      runCount: 0,
      encounterLedger: [],
      snapshotLedger: [],
      receipt: null,
      ...
      noLLM: true,
      phrasebookOnly: true,
      noOpenEndedLanguage: true,
      sourceHistoryPreserved: true,
      noDirectCommand: true,
      noHiddenLawNormalView: true,
      noTechTreeUnlock: true,
      boundary: 'bounded resident encounter; readable dialogue from resident state/history without open-ended language',
    };
  }
  ['encounterLedger', 'snapshotLedger'].forEach(key => {
    if (!Array.isArray(world.gamePrototypeResidentEncounter[key])) world.gamePrototypeResidentEncounter[key] = [];
  });
  return world.gamePrototypeResidentEncounter;
}

function residentEncounterSnapshot(label) {
  ...
  return {
    ...
    resident: world.selected,
    room: world.avatar.room,
    trust: Number(resident.trust || 0),
    debt: Number(resident.debt || 0),
    progress: Number(resident.progress || 0),
    schedule: resident.schedule,
    memory: resident.memory,
    cue: `${expression.marker}: ${expression.reason}`,
    posture: expression.posture,
    body_fatigue: body ? Number(body.fatigue || 0) : null,
    body_position: body ? { ...body.position3d } : null,
    active_proposal: proposal ? `${proposal.proposal_id}: ${proposal.problem_addressed}` : 'none',
    active_practice: practice ? `${practice.practice_id}: ${practice.local_name}` : 'none',
...
function boundedResidentEncounterLine(snapshot, talkPayload, schedulePayload) {
  const trustBand = snapshot.trust < 0.35 ? 'guarded' : snapshot.trust > 0.7 ? 'open' : 'careful';
  const memoryClause = snapshot.memory ? `I remember ${snapshot.memory}` : 'I am watching what happens next';
...
function updatePlayerResidentEncounterAcceptance() {
  const encounter = ensurePlayerResidentEncounter();
  encounter.acceptanceReady = Boolean(
    encounter.encounterLedger.length > 0 &&
    encounter.snapshotLedger.length > 0 &&
    encounter.encounterLedger.every(row => row.player_facing === true && row.no_llm === true && row.phrasebook_only === true && row.open_ended_language === false && row.avatar_direct_command === false && row.hidden_law_normal_view === false && row.tech_tree_unlock === false && row.source_history_preserved === true) &&
    encounter.noLLM === true &&
    encounter.phrasebookOnly === true &&
    encounter.noOpenEndedLanguage === true &&
    encounter.sourceHistoryPreserved === true &&
    encounter.noDirectCommand === true &&
    encounter.noHiddenLawNormalView === true
  );
  encounter.receipt = {
    receipt_id: `PRE-${String(encounter.runCount).padStart(2, '0')}`,
    acceptance_ready: encounter.acceptanceReady,
    encounter_rows: encounter.encounterLedger.length,
    snapshots: encounter.snapshotLedger.length,
    latest_resident: encounter.encounterLedger.length ? encounter.encounterLedger[encounter.encounterLedger.length - 1].resident : 'none',
    boundary: encounter.boundary,
  };
  return encounter.acceptanceReady;
}

function runPlayerResidentEncounterLoop() {
  const encounter = ensurePlayerResidentEncounter();
  encounter.runCount += 1;
  if (!world.entered) runPrototypeOpening();
  if (!world.gamePrototypePlayerMode || !world.gamePrototypePlayerMode.enabled) enterPlayerMode();
  if (!world.gamePrototypeWorldStage || !world.gamePrototypeWorldStage.acceptanceReady) runPrimaryPlaySurfaceStep();
  const before = residentEncounterSnapshot('before-resident-encounter');
  const talkReceipt = talkBounded();
  const scheduleReceipt = askSchedule();
  const after = residentEncounterSnapshot('after-resident-encounter');
  const line = boundedResidentEncounterLine(after, talkReceipt, scheduleReceipt);
  const row = {
    encounter_id: `PRE-${String(encounter.encounterLedger.length + 1).padStart(2, '0')}`,
    tick: world.tick,
    resident: world.selected,
    phrase: after.phrase,
    cue: after.cue,
    ...
    schedule_event: scheduleReceipt && scheduleReceipt.event ? scheduleReceipt.event : 'askSchedule',
    player_facing: true,
    no_llm: true,
    phrasebook_only: true,
    open_ended_language: false,
  ...
  encounter.encounterLedger.push(row);
  if (encounter.encounterLedger.length > 40) encounter.encounterLedger.shift();
  encounter.snapshotLedger.push({ snapshot_id: `PRES-${String(encounter.snapshotLedger.length + 1).padStart(2, '0')}`, tick: world.tick, before, after });
  if (encounter.snapshotLedger.length > 40) encounter.snapshotLedger.shift();
  updatePlayerResidentEncounterAcceptance();
  recordRealityConstraint('player_resident_encounter', {
    resident: row.resident,
    sourceBeliefId: row.encounter_id,
    materials: ['resident_attention', 'source_history', 'bounded_phrasebook'],
    publicObservation: row.response,
    residentInterpretation: row.memory,
    materialTransformation: 'resident encounter changed memory/trust through existing bounded talk and schedule hooks; no material resource spawned',
    timeCost: 1,
    workCost: 0,
    toolWear: 0,
    maintenanceObligation: row.active_proposal !== 'none' ? row.active_proposal : 'preserve resident source history',
    unintendedConsequence: 'bounded conversation can surface refusal, caution, or trust without open-ended language',
    hiddenLawInvolved: 'none in normal view',
    conservationCheck: true
  });
  recordPrototypeMilestone('player-resident-encounter', {
    ready: encounter.acceptanceReady,
    rows: encounter.encounterLedger.length,
    resident: row.resident,
  });
  return log('runPlayerResidentEncounterLoop', { ready: encounter.acceptanceReady, rows: encounter.encounterLedger.length, resident: row.resident, phrase: row.phrase, noLLM: true });
}
...
function formatPlayerResidentEncounter() {
  ...
  return [
    ...
    `Rows: ${encounter.encounterLedger.length} / snapshots=${encounter.snapshotLedger.length}`,
    `Boundary: ${encounter.boundary}`,
    ...
    ...(rows.length ? rows : ['No resident encounter rows yet.'])
  ].join('\n');
}

function ensurePlayerObjectInteraction() {
  ensureGamePrototype();
  if (!world.gamePrototypeObjectInteraction) {
    world.gamePrototypeObjectInteraction = {
      milestone: 'player-object-interaction',
      runCount: 0,
      interactionLedger: [],
      snapshotLedger: [],
      receipt: null,
...
function currentObjectInteractionTarget() {
  const sim = ensurePrototype3DWorld();
  const stage = world.gamePrototypeWorldStage || null;
  const latest = stage && stage.latestSnapshot ? stage.latestSnapshot : currentPrimaryPlaySurfaceSnapshot();
  ...
  const component = sim.components.find(row => row.component_id === activeId)
    ...
    || sim.components[0];
  const term = component ? sim.language.terms.find(row => row.term_id === component.resident_term_id) || sim.language.terms[0] : null;
  const loop = world.gamePrototypeMaterialManipulation || null;
  const latestManipulation = loop && loop.actionLedger.length ? loop.actionLedger[loop.actionLedger.length - 1] : null;
  return { sim, component, term, latestManipulation };
}

function playerObjectInteractionSnapshot(label) {
  ...
  return {
    ...
    resident_term: target.term ? target.term.resident_word : component.resident_term_id || 'none',
    player_gloss: target.term ? target.term.player_gloss : component.material_id || 'none',
    translation_confidence: target.term ? target.term.translation_confidence : 0,
    mass: Number(component.mass || 0),
    position3d: component.position3d ? { ...component.position3d } : null,
    stability: Number(component.stability || 0),
    moisture: Number(component.moisture || 0),
    ...
    field_stress: Number(component.field_stress || 0),
    carried_by: component.carried_by || null,
    latest_manipulation: target.latestManipulation ? target.latestManipulation.manipulation_id : 'none',
    latest_action: target.latestManipulation ? target.latestManipulation.action : 'none',
    latest_success: target.latestManipulation ? target.latestManipulation.success === true : false,
    visible_player_action: 'inspect object and ask what residents can do',
    avatar_direct_command: false,
    resident_chosen: true,
    hidden_law_normal_view: false,
    tech_tree_unlock: false,
...
function updatePlayerObjectInteractionAcceptance() {
  const interaction = ensurePlayerObjectInteraction();
  interaction.acceptanceReady = Boolean(
    interaction.interactionLedger.length > 0 &&
    interaction.snapshotLedger.length > 0 &&
    interaction.interactionLedger.every(row => row.player_facing === true && row.avatar_direct_command === false && row.resident_chosen === true && row.hidden_law_normal_view === false && row.tech_tree_unlock === false && row.manipulation_id !== 'none') &&
    interaction.noDirectCommand === true &&
    interaction.residentChoosesHandling === true &&
    interaction.noHiddenLawNormalView === true &&
    interaction.noTechTreeUnlock === true
  );
  interaction.receipt = {
    receipt_id: `POI-${String(interaction.runCount).padStart(2, '0')}`,
    acceptance_ready: interaction.acceptanceReady,
    interaction_rows: interaction.interactionLedger.length,
...
function runPlayerObjectInteractionLoop() {
  ...
  const row = {
    ...
    tick: world.tick,
    component_id: payload.componentId || before.component_id,
    resident_term: after.resident_term,
    ...
    physics_step_id: payload.physicsStepId || 'none',
    practice_id: payload.practiceId || 'none',
    before,
    after,
    player_facing: true,
    avatar_direct_command: false,
  ...
  interaction.interactionLedger.push(row);
  ...
  recordRealityConstraint('player_object_interaction', {
    resident: row.resident,
    sourceBeliefId: row.interaction_id,
    materials: [row.component_id, after.material_id].filter(Boolean),
    publicObservation: `${row.resident} ${row.resident_action} ${row.resident_term}`,
    residentInterpretation: row.success ? 'resident handling produced usable physical evidence' : 'resident handling preserved a warning',
    materialTransformation: `object interaction changed or observed component ${row.component_id} through resident material manipulation`,
    timeCost: 1,
    workCost: 1,
    toolWear: 0,
    maintenanceObligation: row.practice_id !== 'none' ? row.practice_id : 'watch object handling evidence',
    unintendedConsequence: 'object inspection can route into resident practice evidence without direct command',
    hiddenLawInvolved: 'audit only',
    conservationCheck: true
  });
  recordPrototypeMilestone('player-object-interaction', {
    ready: interaction.acceptanceReady,
...
function ensureFirstPlayableSession() {
  ensureGamePrototype();
  if (!world.gamePrototypePlaySession) {
    world.gamePrototypePlaySession = {
      milestone: 'first-playable-session',
      runCount: 0,
      stepLedger: [],
      snapshotLedger: [],
      receipt: null,
      acceptanceReady: false,
      requiredSteps: ['enter_player_mode', 'look', 'move', 'talk', 'objects', 'material_handling', 'proposal_deck', 'practice_loop', 'resident_worksite', 'return_journal', 'save', 'return'],
      playerFacing: true,
      noDirectCommand: true,
      noHiddenLawNormalView: true,
      noTechTreeUnlock: true,
      boundary: 'single player-facing session receipt; sequences normal play surfaces without adding world law',
    };
  }
  const preferredSessionSteps = ['enter_player_mode', 'look', 'move', 'talk', 'objects', 'material_handling', 'proposal_deck', 'practice_loop', 'resident_worksite', 'return_journal', 'save', 'return'];
...
function cloneFirstPlayableSession(session) {
  return {
    ...session,
    stepLedger: session.stepLedger.slice(),
...
function firstPlayableSessionSnapshot(label) {
  const guide = derivePrototypePlayerGuide();
  const proposal = world.villageBoard && world.villageBoard.projectProposals.length ? world.villageBoard.projectProposals[world.villageBoard.projectProposals.length - 1] : null;
  const practice = world.emergentPracticeGraph && world.emergentPracticeGraph.nodes.length ? world.emergentPracticeGraph.nodes[world.emergentPracticeGraph.nodes.length - 1] : null;
  const saves = world.gamePrototypeSaves || null;
  ...
  return {
    ...
    guide_action: guide.nextAction,
    player_mode_enabled: world.gamePrototypePlayerMode ? world.gamePrototypePlayerMode.enabled === true : false,
    action_rail_ready: world.gamePrototypeActionRail ? world.gamePrototypeActionRail.acceptanceReady === true : false,
    movement_route_ready: world.gamePrototypeMovementRoute ? world.gamePrototypeMovementRoute.acceptanceReady === true : false,
    ...
    save_returns: saves ? saves.returnLog.length : 0,
    forward_returns: returnLater ? returnLater.returnLedger.length : 0,
    latest_practice: practice ? `${practice.practice_id}: ${practice.local_name}` : 'none',
    latest_proposal: proposal ? `${proposal.proposal_id}: ${proposal.problem_addressed}` : 'none',
...
function updateFirstPlayableSessionAcceptance() {
  ...
  session.acceptanceReady = Boolean(
    ...
    session.stepLedger.every(row => row.player_facing === true && row.avatar_direct_command === false && row.hidden_law_normal_view === false && row.tech_tree_unlock === false) &&
    world.gamePrototypePlayerMode && world.gamePrototypePlayerMode.acceptanceReady === true &&
    world.gamePrototypeMovementRoute && world.gamePrototypeMovementRoute.acceptanceReady === true &&
    ...
    session.noDirectCommand === true &&
    session.noHiddenLawNormalView === true &&
    session.noTechTreeUnlock === true
  );
  session.receipt = {
    receipt_id: `FPS-${String(session.runCount).padStart(2, '0')}`,
    acceptance_ready: session.acceptanceReady,
    step_rows: session.stepLedger.length,
...
function recordFirstPlayableSessionStep(stepId, actionLabel, actionFn) {
  const preserved = cloneFirstPlayableSession(ensureFirstPlayableSession());
  const before = firstPlayableSessionSnapshot(`before-${stepId}`);
  const result = actionFn();
  const session = restoreFirstPlayableSessionAfterWorldSwap(preserved);
  const after = firstPlayableSessionSnapshot(`after-${stepId}`);
  const row = {
    step_id: stepId,
    action_label: actionLabel,
    result_event: result && result.event ? result.event : stepId,
    tick: world.tick,
    before,
    after,
  ...
  session.stepLedger.push(row);
  if (session.stepLedger.length > 40) session.stepLedger.shift();
  session.snapshotLedger.push({
    snapshot_id: `FPS-S-${String(session.snapshotLedger.length + 1).padStart(2, '0')}`,
    tick: world.tick,
    snapshot: after,
  });
  if (session.snapshotLedger.length > 40) session.snapshotLedger.shift();
  updateFirstPlayableSessionAcceptance();
  return row;
}

function runFirstPlayableSessionLoop() {
  const session = ensureFirstPlayableSession();
  session.runCount += 1;
  recordFirstPlayableSessionStep('enter_player_mode', 'Enter player mode', () => {
    if (!world.entered) runPrototypeOpening();
    return runPlayerModeInterfaceLoop();
  });
  recordFirstPlayableSessionStep('look', 'Look', runNormalPlayLook);
  recordFirstPlayableSessionStep('move', 'Move', runPlayerMovementRouteLoop);
  recordFirstPlayableSessionStep('talk', 'Talk', runPlayerResidentEncounterLoop);
  recordFirstPlayableSessionStep('objects', 'Objects', runPlayerObjectInteractionLoop);
  recordFirstPlayableSessionStep('material_handling', 'Handling', runNormalPlayHandling);
  recordFirstPlayableSessionStep('proposal_deck', 'Proposals', runPlayerProposalDeckLoop);
  ...
  recordPrototypeMilestone('first-playable-session', {
    ready: updatedSession.acceptanceReady,
    steps: updatedSession.stepLedger.length,
    snapshots: updatedSession.snapshotLedger.length,
  });
  return log('runFirstPlayableSessionLoop', { ready: updatedSession.acceptanceReady, steps: updatedSession.stepLedger.length, snapshots: updatedSession.snapshotLedger.length });
}
...
function formatFirstPlayableSession() {
  ...
  return [
    ...
    ...(steps.length ? steps : ['No first playable session rows yet.'])
  ].join('\n');
}
...
function derivePrototypePlayerGuide() {
  ...
  if (!world.gamePrototypeActionRail || !world.gamePrototypeActionRail.acceptanceReady) {
    return { ...guide, phase: 'normal play controls', nextAction: 'Normal play loop', why: 'prove the prototype can be driven through player-language verbs: Look, Move, Ask, Talk, Objects, Handling, Support, Wait, Return, and Save', button: 'runNormalPlayActionRailLoop' };
  }
  ...
  if (!world.gamePrototypeMovementRoute || !world.gamePrototypeMovementRoute.acceptanceReady) {
    return { ...guide, phase: 'movement route', nextAction: 'Move', why: 'move through village space and update nearby affordances from the current zone', button: 'runPlayerMovementRouteLoop' };
  }
  if (!world.gamePrototypeResidentEncounter || !world.gamePrototypeResidentEncounter.acceptanceReady) {
    return { ...guide, phase: 'resident encounter', nextAction: 'Talk', why: 'show a bounded source-traced resident response from memory, schedule, visible cue, proposal, practice, and object context', button: 'runPlayerResidentEncounterLoop' };
  }
  if (!world.gamePrototypeObjectInteraction || !world.gamePrototypeObjectInteraction.acceptanceReady) {
    return { ...guide, phase: 'physical object interaction', nextAction: 'Objects', why: 'inspect a physical component and let residents choose a handling response under material constraints', button: 'runPlayerObjectInteractionLoop' };
  }
  if (!world.gamePrototypeMaterialManipulation || !world.gamePrototypeMaterialManipulation.actionLedger.length) {
    return { ...guide, phase: 'material handling', nextAction: 'Handling', why: 'watch residents physically move, tie, dry, wet-test, stack, or test components through material constraints', button: 'runNormalPlayHandling' };
  }
  ...
  if (!world.gamePrototypeReturnJournal || !world.gamePrototypeReturnJournal.acceptanceReady) {
    return { ...guide, phase: 'return journal', nextAction: 'Return journal', why: 'show what changed while away and what a save-slot return restored from source history', button: 'runReturnJournalLoop' };
  }
  if (!world.gamePrototypePlaySession || !world.gamePrototypePlaySession.acceptanceReady) {
...
function runPrototypeQASmoke() {
  ...
  const savedCounts = {
    ...
    movementRouteReady: world.gamePrototypeMovementRoute ? world.gamePrototypeMovementRoute.acceptanceReady === true : false,
    movementRouteRows: world.gamePrototypeMovementRoute ? world.gamePrototypeMovementRoute.routeLedger.length : 0,
    movementRouteSnapshots: world.gamePrototypeMovementRoute ? world.gamePrototypeMovementRoute.snapshotLedger.length : 0,
    residentEncounterReady: world.gamePrototypeResidentEncounter ? world.gamePrototypeResidentEncounter.acceptanceReady === true : false,
    residentEncounterRows: world.gamePrototypeResidentEncounter ? world.gamePrototypeResidentEncounter.encounterLedger.length : 0,
    residentEncounterSnapshots: world.gamePrototypeResidentEncounter ? world.gamePrototypeResidentEncounter.snapshotLedger.length : 0,
    objectInteractionReady: world.gamePrototypeObjectInteraction ? world.gamePrototypeObjectInteraction.acceptanceReady === true : false,
    objectInteractionRows: world.gamePrototypeObjectInteraction ? world.gamePrototypeObjectInteraction.interactionLedger.length : 0,
    objectInteractionSnapshots: world.gamePrototypeObjectInteraction ? world.gamePrototypeObjectInteraction.snapshotLedger.length : 0,
    proposalDeckReady: world.gamePrototypeProposalDeck ? world.gamePrototypeProposalDeck.acceptanceReady === true : false,
...
function savePrototypeSlot(label = 'manual prototype save') {
  ...
  const slot = {
    ...
    project_visual_rows: world.gamePrototypeProjects && world.gamePrototypeProjects.visualLedger ? world.gamePrototypeProjects.visualLedger.length : 0,
    latest_project_visual_id: latestProjectVisual ? latestProjectVisual.visual_id : 'none',
    latest_project_visual_cue: latestProjectVisual ? latestProjectVisual.canvas_cue : 'none',
...
function buildPrototypeAcceptanceReceipt() {
  ...
  const movementRouteSnapshots = movementRoute ? movementRoute.snapshotLedger.length : 0;
  const residentEncounterRows = residentEncounter ? residentEncounter.encounterLedger.length : 0;
...
function renderGamePrototypeSurface() {
  ...
  const movementRouteNode = document.getElementById('gamePrototypeMovementRouteOut');
  const residentEncounterNode = document.getElementById('gamePrototypeResidentEncounterOut');
  const objectInteractionNode = document.getElementById('gamePrototypeObjectInteractionOut');
  ...
  const returnJournalNode = document.getElementById('gamePrototypeReturnJournalOut');
  const playSessionNode = document.getElementById('gamePrototypePlaySessionOut');
  ...
  if (movementRouteNode) movementRouteNode.textContent = formatPlayerMovementRoute();
  if (residentEncounterNode) residentEncounterNode.textContent = formatPlayerResidentEncounter();
  if (objectInteractionNode) objectInteractionNode.textContent = formatPlayerObjectInteraction();
  ...
  if (returnJournalNode) returnJournalNode.textContent = formatReturnJournal();
  if (playSessionNode) playSessionNode.textContent = formatFirstPlayableSession();
...
function describeReplayRow(row) {
  ...
  const descriptions = {
    ...
    savePrototypeSlot: `prototype save ${payload.slotId} year=${payload.year} day=${payload.autonomousDay} projectVisuals=${payload.projectVisualRows || 0}/${payload.latestProjectVisualId || 'none'}`,
    returnPrototypeSlot: `prototype return restored=${payload.restored === true} ${payload.slotId || payload.reason || ''} projectVisuals=${payload.projectVisualRows || 0}/${payload.latestProjectVisualId || 'none'}`,
    ...
    runReturnJournalLoop: `return journal ready=${payload.ready === true} rows=${payload.rows} forward=${payload.forwardReturn} slot=${payload.slotId}`,
    runFirstPlayableSessionLoop: `first playable session ready=${payload.ready === true} steps=${payload.steps} snapshots=${payload.snapshots}`,
